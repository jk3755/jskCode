...
cleanGRanges <- function(ranges, rm.chrM = TRUE, rm.chrY = TRUE, rm.chrX = FALSE){
...
} # end importNarrowpeakAsGRanges

#### Retrieve a TSS window for all genes ####
getGenePromoterWindows <- function(window = 500){
  
  gene_ranges <- generateAllGeneAnnotationsAsGRanges()
  gene_ranges <- promoters(gene_ranges, upstream = window, downstream = window)
  return(gene_ranges)
  
} # end getGenePromoterWindows
